...
image_list = np.loadtxt(os.path.join(DATA_DIR, 'metadata', 'microfibers', 'training.lst'), dtype='str', delimiter=',')

reds, greens, blues = [], [], []
for image_name in image_list:
    # Convert image to rgb if needed (bjornorri)
    im = Image.open(os.path.join(IMG_DIR, image_name + '_rgb.png'))
    im = im.convert('RGB')
...
with open(os.path.join(DATA_DIR, 'metadata', 'microfibers', 'means_and_stds.txt'), 'w') as fp:
